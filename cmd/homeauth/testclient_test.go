...
func assertSessionFor(tb testing.TB, idp *idpServer, token string, wantUserUUID string) {
	...
	sctx, err := idp.smgr.Load(context.Background(), token)
	if err != nil {
		tb.Fatalf("failed to load session: %v", err)
	}
	userUUID := idp.smgr.GetString(sctx, skeyUserUUID)
	if userUUID != wantUserUUID {
		tb.Fatalf("expected session for user %q, got %q", wantUserUUID, userUUID)
	}
}
